			...
			<div id="content">
				...
				<div id="box">
					...
					<?php
							...
							$query = "SELECT library FROM books WHERE idbooks=".$_POST['update'];
							$library_res = mysqli_query($connection,$query);
							$library_array = mysqli_fetch_all($library_res);
							$library_name = $library_array[0][0];

							$query2 = "SELECT rental_duration FROM library WHERE name='".$library_name."'";
							$duration_res = mysqli_query($connection,$query2);
							$duration_array = mysqli_fetch_all($duration_res);
							$duration = $duration_array[0][0];

							$query3 = "SELECT expiration_date FROM rented_books WHERE user_id=".$_SESSION['id']." AND book_id=".$_POST['update'];

							

							$expiration_res = mysqli_query($connection,$query3);
							$expiration_array = mysqli_fetch_all($expiration_res);

							$expiration_date = $expiration_array[0][0];

							$days_to_add = $duration;
							$days_to_add = $days_to_add." days";

							$date = date_create($expiration_date);
							date_add($date,date_interval_create_from_date_string($days_to_add));

							$query4 = "UPDATE rented_books SET expiration_date='".$date->format('y-m-d')."' WHERE user_id = ".$_SESSION['id']." AND book_id = ".$_POST['update'];
							
							$update_res = mysqli_query($connection,$query4);
							if($update_res == FALSE){
								echo "Σφάλμα στην ανανέωση δανεισμού!!";
							}

							else{
								echo "Επιτυχής ανανέωση δανεισμού!!"."<br>"."Νέα ημερομηνία λήξης δανεισμού: ".$date->format('y-m-d');
							}
						?>
